...
t_vec3f ray_color(const t_ray r, const t_hittables *htbl, uint8_t depth)
{
	float			t[2];
	t_hit_record	hr;
	const t_vec3f	albedo = {1.0, 1.0, 1.0};
	t_ray			new_ray;
	int				ret;
	...
	if (ret > -1)
	{
		...
		t_vec3f new_dir = vv_add(hr.normal, random_unit_vector());
		if (fabsf(new_dir.x) < 1e-8 && fabsf(new_dir.y) < 1e-8 && fabsf(new_dir.z) < 1e-8)
			new_dir = hr.normal;
		//The next code is for metallic surfaces
		if (hr.material == metallic)
			new_ray.direction = reflect(new_dir, hr.normal);
		else
			new_ray.direction = new_dir;
		new_ray.origin = hr.hitpoint;
		//t_vec3f attenuation = albedo;
	...
	return vv_add(vt_multiply(albedo, (1.0 - a)), vt_multiply(gradient, a));
}
...
int main()
{
	...
	t_sphere sphere1 = {{0,0,-3},{0,0,0,255},0.5, metallic, sphere};
	t_sphere sphere2 = {{0,-100.5,-3},{0,0,0,255},100, metallic, sphere};
	t_sphere *spheres = malloc(sizeof(t_sphere) * hittables.sphere_count);
	spheres[0] = sphere1;
	spheres[1] = sphere2;
